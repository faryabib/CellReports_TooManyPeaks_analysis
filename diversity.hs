...
newtype ARI = ARI { unARI :: Double } deriving (Show)
newtype Homogeneity = Homogeneity { unHomogeneity :: Double } deriving (Show)
...
-- | Get the adjusted rand index of a clustering.
clusteringToARI :: ClusterMap -> ARI
clusteringToARI (ClusterMap clusterMap) = ARI
                                        . NLP.ari
                                        . uncurry NLP.counts
                                        . unzip
                                        . concatMap (\(!c, xs) -> zip (repeat c) . F.toList $ xs)
...
-- | Get the homogeneity (Rosenberg and Hirschberg, 2007, but fixing typo in HC
-- for n -> N) of a clustering.
clusteringToHomogeneity :: ClusterMap -> Homogeneity
clusteringToHomogeneity (ClusterMap clusterMap) = Homogeneity
                                        . bool (1 - (hck / hc)) 1
                                        $ hck == 0
  where
    n = fromIntegral . Seq.length . mconcat . fmap snd . Map.toAscList $ clusterMap
    sumFold = Map.foldl' (+) 0
    hck = negate
        . sumFold
        . Map.map (\m -> sumFold . Map.map (\ack -> (ack / n) * log (ack / sumFold m)) $ m)
        $ clusterClassMap
    hc =
      negate . sumFold . Map.map (\x -> (x / n) * log (x / n)) $ classSizeMap
    classes = Set.fromList . F.toList . mconcat . Map.elems $ clusterMap
    nClass = fromIntegral $ Set.size classes
    clusterClassMap =
      Map.map (Map.fromListWith (+) . flip zip [1,1..] . F.toList) clusterMap
    classSizeMap =
      Map.fromListWith (+) . flip zip [1,1..] . F.toList . mconcat . Map.elems $ clusterMap

-- | Get the RAGI of the clustering output.
toRAGI :: FP.FilePath -> FP.FilePath -> IO ()
toRAGI scoreFile clusterFile =
  procs
    "/home/gw/env3/bin/python"
    [ "/home/gw/code/single_cell/python/RAGI.py"
    , format fp scoreFile
    , "False"
    , format fp clusterFile
    ]
    $ mempty
...
main = (>>) (putStrLn "label,type,value") $ sh $ do
  ...
  let label = parseFileName . format fp $ file
      ...
      ari = clusteringToARI clusterMap
      homogeneity = clusteringToHomogeneity clusterMap
  ...
  liftIO $ T.putStrLn $ T.intercalate "," [unLabel label, "ARI", showt $ unARI ari]
  liftIO $ T.putStrLn $ T.intercalate "," [unLabel label, "Homogeneity", showt $ unHomogeneity homogeneity]
  liftIO $ toRAGI "/path/to/activities.csv" file
